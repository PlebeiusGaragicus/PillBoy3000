...
class MicroSD(Singleton, BaseThread):
    ...
    def is_inserted(self):
        from pillboy.models.settings import Settings  # Import here to avoid circular import issues

        if Settings.HOSTNAME == Settings.PILLBOY_OS:
    ...
    def run(self):
        ...
        from pillboy.gui.toast import SDCardStateChangeToastManagerThread
        from pillboy.models.settings import Settings  # Import here to avoid circular import issues
        action = ""

        # explicitly only microsd add/remove detection in pillboy-os
        if Settings.HOSTNAME == Settings.PILLBOY_OS:
